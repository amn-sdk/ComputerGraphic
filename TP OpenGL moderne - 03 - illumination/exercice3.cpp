...
GLuint CreateCheckerTexture()
{
    ...
    for (int y = 0; y < SIZE; y++)
    {
        for (int x = 0; x < SIZE; x++)
        {
            ...
            int idx = (y * SIZE + x) * 3;
            if (checker == 0)
            { pixels[idx]=190; pixels[idx+1]=110; pixels[idx+2]=50; }
            else
            { pixels[idx]=90;  pixels[idx+1]=50;  pixels[idx+2]=20; }
        }
    }
    GLuint texId;
...
void Render(GLFWwindow* window)
{
    ...
    float lightDir[3]       = { 0.f, -1.f, 1.f };  
    float lightDiffColor[3] = { 1.f,  1.f, 1.f };  
    float lightSpecColor[3] = { 1.f,  1.f, 1.f };  
    glUniform3fv(glGetUniformLocation(program, "u_lightDir"),       1, lightDir);
    glUniform3fv(glGetUniformLocation(program, "u_lightDiffColor"), 1, lightDiffColor);
    glUniform3fv(glGetUniformLocation(program, "u_lightSpecColor"), 1, lightSpecColor);
    float Ks[3] = { 1.f, 1.f, 1.f }; 
    glUniform3fv(glGetUniformLocation(program, "u_Ks"), 1, Ks);
    glUniform1f(glGetUniformLocation(program, "u_shininess"), 16.f); 
    float camPos[3] = { 0.f, 0.f, 0.f };
    glUniform3fv(glGetUniformLocation(program, "u_cameraPos"), 1, camPos);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(960, 540,
        "Exercice 3 - Speculaire Phong (Dragon)", NULL, NULL);
