...
vec3 g_CameraTargets[3]={vec3(0.0f, -1.4f, -1.5f), vec3(6.0f, -1.4f, -1.5f), vec3(-6.0f, -1.4f, -1.5f)};
bool g_Keys[1024]={};
...
void Render(GLFWwindow* window)
{
    ...
    glUniform3f(glGetUniformLocation(pScene,"u_cameraPos"),g_Camera.position.x,g_Camera.position.y,g_Camera.position.z);
...

    {
        ...
        for (int i=0;i<NB_INSTANCES;i++) {
            ...
            if (i==0) g_CameraTargets[1]=vec3(px,-1.4f,pz-1.5f);
            else if (i==2) g_CameraTargets[2]=vec3(px,-1.4f,pz-1.5f);
        }
        if (!g_Camera.freeMode) {
...
void key_callback(GLFWwindow* window,int key,int scancode,int action,int mods)
{
    ...
    if (action==GLFW_PRESS)
    {
        ...
        if (key==GLFW_KEY_3) { g_PostProcessMode=2; 
        std::cout << "Post-process : Sepia"      << std::endl; }
        if (key==GLFW_KEY_4) { g_PostProcessMode=3; 
        std::cout << "Post-process : Inversion"  << std::endl; }
        if (key==GLFW_KEY_I) { g_BackLightStrength+=0.2f; 
        std::cout << "Fresnel : " << g_BackLightStrength << std::endl; }
