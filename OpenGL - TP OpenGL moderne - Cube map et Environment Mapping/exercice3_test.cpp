...
GLuint LoadCubemap()
{
    // Ordre : +X, -X, +Y, -Y, +Z, -Z
    const char* faces[6] = {
        "envmaps/test_px.png", "envmaps/test_nx.png",
        "envmaps/test_py.png", "envmaps/test_ny.png",
        "envmaps/test_pz.png", "envmaps/test_nz.png"
    };
    ...
    // Important pour les images dont la largeur n'est pas multiple de 4
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

    for (int i = 0; i < 6; i++)
    {
        int w, h, nChannels;
        // On force 3 canaux (RGB) pour eviter les problemes de format
        unsigned char* data = stbi_load(faces[i], &w, &h, &nChannels, 3);
        if (data)
        {
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                         0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
            
            std::cout << "Face " << i << " chargee (" << w << "x" << h << ") : " << faces[i] << std::endl;
...
bool Initialise()
{
    ...
    std::cout << "- R : mode Reflexion" << std::endl;
    std::cout << "- F : mode Refraction" << std::endl;
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(960, 540,
        "TP Cube Mapping - Exercice 3 Test (Cube + Test Maps)", NULL, NULL);
