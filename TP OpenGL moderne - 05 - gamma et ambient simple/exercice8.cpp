...
// --- Variables globales pour la caméra orbitale ---
bool g_isMousePressed = false;
double g_lastMouseX = 0.0;
double g_lastMouseY = 0.0;
float g_azimuth = 0.0f;
float g_elevation = 0.0f;
float g_radius = 15.0f; // Distance de la caméra
// --------------------------------------------------
...
bool Initialise()
{
    g_Shader.LoadVertexShader("exo8.vs");
    g_Shader.LoadFragmentShader("exo8.fs");
    g_Shader.Create();
...
void Render(GLFWwindow* window)
{
    ...
    // Dragon tourne automatiquement autour de Y pour la vidéo
    Mat4 rotY  = makeRotationY(time * 0.5f);
    // Le dragon est légèrement abaissé et plus loin pour tout voir
    Mat4 trans = makeTranslation(0.f, -1.5f, -18.f);
    ...
    // La caméra est fixe, positionnée à l'origine
    ...
    // Pour les normales : on passe la rotation seule (sans translation)
    ...
    setUniform3f(program, "u_cameraPos", camX, camY, camZ);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(960, 540,
        "Illumination Hemisphérique - Sky/Ground Ambient", NULL, NULL);
