...
float g_camAzimuth = 3.14159265f / 2.0f; 
float g_camElevation = 0.0f;             
float g_camRadius = 20.0f;               
...
void cursor_position_callback(GLFWwindow* window,double xpos,double ypos)
{
    if (g_isDragging)
    {
        ...
        g_camAzimuth+=(float)dx*0.01f;
        g_camElevation+=(float)dy*0.01f;
        float limit=3.14159265f/2.0f-0.01f;
        if(g_camElevation>limit){
            g_camElevation=limit;
        }
        else if(g_camElevation<-limit){
            g_camElevation=-limit;
        }
        g_lastMouseX=xpos;
        g_lastMouseY=ypos;
...
{
    g_camRadius-=(float)yoffset*1.0f;
...
void Render(GLFWwindow* window)
{
    ...
    Mat4 scale= makeScale(1.f,1.f,1.f);
    Mat4 rotY=makeRotationY(time*0.8f);
    Mat4 trans= makeTranslation(0.f,-2.f,0.f);
    Mat4 world = multiply(trans,multiply(rotY, scale));
    float Y =g_camRadius*sinf(g_camElevation);
    float X =g_camRadius*cosf(g_camElevation)*cosf(g_camAzimuth);
    float Z=g_camRadius*cosf(g_camElevation)*sinf(g_camAzimuth);
    vec3 camPos(X,Y,Z);
    vec3 camTarget(0.f,-2.f,0.f);
    vec3 camUp(0.f,1.f,0.f);
    Mat4 view= makeLookAt(camPos,camTarget,camUp);
    float aspect=1.f;
    if (height>0){
        aspect=(float)width/(float)height;
    }
    Mat4 proj= makePerspective(60.f,aspect,0.1f,1000.f);
    Mat4 mvp= multiply(proj,multiply(view, world));
    auto program = g_Shader.GetProgram();
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(960, 540,
        "TP Preparation - Exercice 3", NULL, NULL);
